...
push() {
  ...
  # this means that an attacker must, for each individual, record all communications,
  #   and break a chain of public keys, because only the first public keys are transfered unencrypted
  ...
  # create hard'links in the "~/Public/project_name"
...
pull() {
  ...
  # sync down the files mentioned in the index file (wget2 -c ...)
...
backup() {
  ...
  # if $1 is a path, do backup only for that path
