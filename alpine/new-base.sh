...
echo "set root password (can be the same one entered before to encrypt the root partition)"
chroot "$new_root" passwd root
...
chroot "$new_root" adduser home plugdev
chroot "$new_root" adduser home gnunet
...
echo '#!/usr/bin/env sh
setpriv --reuid=$(id -u home) --regid=$(id -g home) --groups=gnunet $@
' > "$new_root"/usr/local/bin/udo
chmod +x "$new_root"/usr/local/bin/udo

mkdir -p /etc/doas.d
cat <<-EOF > /etc/doas.d/shell.conf
permit nopass home cmd /usr/local/bin/udo
...
# 	that privilage will be dropped (using "udo") for all launched apps and commands
