...
mkdir -p "$project_path"/.cache/mkdi
...
rm -f "$project_path"/.cache/mkdi/"$initrd_rpath"
find . | bsdcpio -oz --format=newc > "$project_path"/.cache/mkdi/"$initrd_rpath"
rm -rf ./*
# do the same for "gtk/initrd.gz"
bsdcat ../"$initrd_gtk_rpath" | bsdcpio -i
cp "$project_path"/mkdi/preseed.cfg .
echo 'export DEBIAN_FRONTEND=text' > lib/debian-installer.d/S99text-frontend
rm -f "$project_path"/.cache/mkdi/"$initrd_gtk_rpath"
find . | bsdcpio -oz --format=newc > "$project_path"/.cache/mkdi/"$initrd_gtk_rpath"
cd "$project_path"/.cache/mkdi
rm -rf initrd
...
sh "$project_path"/os/sd write debian-modified-"$1"-netinst.iso "$2"
